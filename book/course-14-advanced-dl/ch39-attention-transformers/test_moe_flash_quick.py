"""
Quick test for MoE and Flash Attention (reduced epochs for speed)
"""
...
# Block 5: Mixture of Experts (REDUCED EPOCHS)
def block5():
    import torch
    ...
    # Training function (REDUCED TO 20 EPOCHS)
    def train_model(model, X_train, y_train, X_test, y_test, is_moe=False, epochs=20):
...
results.append(test_block(5, "Mixture of Experts (Quick Test)", block5))

# Block 6: Flash Attention (Quick test with fewer iterations)
def block6():
    import torch
    import torch.nn.functional as F
    import math
    ...
    # Quick benchmark on smaller sequences
    ...
    for seq_len in seq_lengths:
        ...
        standard_time = (time.time() - start) / 5

        # Benchmark PyTorch's optimized attention
        start = time.time()
        for _ in range(5):
            _ = F.scaled_dot_product_attention(Q, K, V)
        optimized_time = (time.time() - start) / 5

        # Memory usage (attention matrix size)
        attention_matrix_size = batch_size * num_heads * seq_len * seq_len * 4
    ...
    print("\n" + "=" * 60)
    print("Key Takeaways:")
    print("- Flash Attention provides exact attention with reduced memory")
    print("- Speedup increases with sequence length")
    print("- Enables longer contexts and larger batch sizes")

results.append(test_block(6, "Flash Attention (Quick Test)", block6))
